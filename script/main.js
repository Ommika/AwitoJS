...
//Закрытие модальных окон (общая функция 1)
...
const closeModalEsc = event => {
  if (event.key === 'Escape') {
    ...
    modalItem.classList.add('hide');
    modalSubmit.reset();
    document.removeEventListener('keydown', closeModal); //было closeModalEsc
  }
};
...
addAd.addEventListener('click', () => {
  modalAdd.classList.remove('hide');
  modalBtnSubmit.disabled = true;
  document.addEventListener('keydown', closeModal); //было closeModalEsc
});
modalAdd.addEventListener('click', closeModal);
//Открытие/закрытие модального окна объявления
